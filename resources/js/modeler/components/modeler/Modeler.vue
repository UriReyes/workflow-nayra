<template>
    <span data-test="body-container">
        ...
        <b-row class="modeler h-100">
            ...
            />
            <b-col
                class="pr-4 paper-container h-100"
                ref="paper-container"
...
import { saveProcess } from "@/aworkflow/api";
...
export default {
    ...
    watch: {
        isRendering() {
            ...
            window.ProcessMaker.navbar.alerts =
                window.ProcessMaker.navbar.alerts.filter((alert) => {
    ...
    computed: {
        noElementsSelected() {
            return (
                this.highlightedNodes.filter(
                    (node) => !node.isType("processmaker-modeler-process")
                ).length === 0
            );
        },
        tooltipTitle() {
    ...
    methods: {
        ...
        async saveBpmn() {
            ...
            this.saveInBackend(this.processId, xml, svgString);
            this.$emit("saveBpmn", { xml, svg: svgString });
        },
        borderOutline(nodeId) {
            return (
                this.decorations.borderOutline &&
                this.decorations.borderOutline[nodeId]
            );
        },
        addWarning(warning) {
        ...
        validatePlaneElements() {
            this.planeElements
                ...
                .map((diagram) => ({
                    ...
                    text:
                        this.$t("bpmdi:BPMNShape ") +
                        diagram.id +
                        this.$t(
                            " references a non-existent element and was not parsed"
                        ),
                }))
        ...
        async validateBpmnDiagram() {
            if (
                !store.getters.globalProcesses ||
                store.getters.globalProcesses.length === 0
            ) {
        ...
        highlightNode(node, event) {
            ...
            let isSameHighlightedNode = _.isEqual(
                node.id,
        ...
        registerNode(nodeType, customParser) {
            ...
            const implementationParser = (definition) => {
                if (
                    definition.get("implementation") === nodeType.implementation
                ) {
                    return nodeType.id;
                }
                return undefined;
            ...
            types.forEach((bpmnType) => {
                ...
                if (nodeType.implementation) {
                    this.parsers[bpmnType].implementation.push(
        ...
        loadAssociations(flowElements, artifacts) {
            artifacts
                .filter(
                    (definition) =>
                        definition.$type === "bpmn:Association" &&
                        this.hasSourceAndTarget(definition)
                )
                .forEach((definition) =>
                    this.setNode(definition, flowElements, artifacts)
                );
        },
        loadDataAssociations(flowElements) {
            const tasksThatHaveDataOutputAssociations = flowElements.filter(
                (task) =>
            ...
            tasksThatHaveDataOutputAssociations.forEach((task) => {
                task.get("dataOutputAssociations").forEach(
                    (dataAssociationLink) => {
            ...
            const tasksThatHaveDataInputAssociations = flowElements.filter(
                (task) =>
                    task.get("dataInputAssociations") &&
                    task.get("dataInputAssociations").length > 0
            );

            tasksThatHaveDataInputAssociations.forEach((task) => {
                task.get("dataInputAssociations").forEach(
                    (dataAssociationLink) => {
                        this.setNode(dataAssociationLink, flowElements);
                    }
                );
        ...
        loadSequenceFlows(flowElements, artifacts) {
            flowElements
                .filter(
                    (definition) =>
                        definition.$type === "bpmn:SequenceFlow" &&
                        this.hasSourceAndTarget(definition)
                )
                .forEach((definition) =>
                    this.setNode(definition, flowElements, artifacts)
        ...
        loadFlowElements(flowElements, artifacts) {
            flowElements
                .filter(
                    (definition) => definition.$type !== "bpmn:SequenceFlow"
                )
                .forEach((definition) =>
                    this.setNode(definition, flowElements, artifacts)
                );
        ...
        getProcesses() {
            return this.definitions.rootElements.filter(
        ...
        parse() {
            ...
            this.processNode = new Node(
                ...
                this.processes[0],
                this.planeElements.find(
                    (diagram) => diagram.bpmnElement.id === this.processes[0].id
                )
            );
        },
        removeUnsupportedElementAttributes(definition) {
            ...
            unsupportedElements
                .filter((name) => definition.get(name))
        ...
        getCustomParser(definition) {
            ...
            const customParser = parsers.custom.find((parser) =>
                parser(definition, this.moddle)
            );
            const implementationParser = parsers.implementation.find((parser) =>
                parser(definition, this.moddle)
            );
            const defaultParser = parsers.default.find((parser) =>
                parser(definition, this.moddle)
            );
        ...
        handleUnsupportedElement(
            ...
            this.addWarning({
                ...
                text:
                    bpmnType +
                    this.$t(" is an unsupported element type in parse"),
            });
        ...
        setNode(definition, flowElements, artifacts) {
            const diagram = this.planeElements.find(
                (diagram) => diagram.bpmnElement.id === definition.id
            );
            const bpmnType = definition.$type;
            const parser = this.getCustomParser(definition);

            if (!parser) {
                this.handleUnsupportedElement(
                    bpmnType,
                    flowElements,
                    definition,
                    artifacts,
        ...
        hasSourceAndTarget(definition) {
            const hasSource =
                ...
                this.parsers[definition.sourceRef.$type];
            const hasTarget =
                definition.targetRef &&
                this.parsers[definition.targetRef.$type];
        ...
        async waitForCursorToChange() {
            ...
            return new Promise((resolve) =>
                setTimeout(resolve, cursorWaitTime)
            );
        },
        async renderPaper() {
        ...
        getBoundaryEvents(process) {
            return process
                .get("flowElements")
                .filter(({ $type }) => $type === "bpmn:BoundaryEvent");
        },
        createBoundaryEvent(definition) {
            const boundaryEvent = boundaryEventConfig.definition(
                this.moddle,
                this.$t
            );
            ...
            boundaryEvent.set(
                "eventDefinitions",
                definition.get("eventDefinitions")
            );
            boundaryEvent.set(
                "cancelActivity",
                definition.get("cancelActivity")
            );
        ...
        async handleDrop({
            ...
            const definition = this.nodeRegistry[control.type].definition(
                this.moddle,
                this.$t
            );

            const diagram = this.nodeRegistry[control.type].diagram(
                this.moddle
            );

            const { x, y } = this.paperManager.clientToGridPoint(
                clientX,
                clientY
            );
            diagram.bounds.x = x;
        ...
        addNode(node) {
            ...
            const targetProcess = node.getTargetProcess(
                this.processes,
                this.processNode
            );
            ...
            this.performSingleUndoRedoTransaction(async () => {
                await this.paperManager.performAtomicAction(async () => {
                    const { x: clientX, y: clientY } =
                        this.paper.localToClientPoint(node.diagram.bounds);
        ...
        removeNodesFromPool(node) {
            if (
                node.type === "processmaker-modeler-pool" &&
                node.definition.processRef
            ) {
                if (node.definition.processRef.artifacts) {
                    node.definition.processRef.artifacts.forEach((artifact) => {
                        const nodeToRemove = this.nodes.find(
                            (n) => n.definition === artifact
                        );
                        if (nodeToRemove) {
                ...
                if (node.definition.processRef.flowElements) {
                    node.definition.processRef.flowElements.forEach(
                        (flowElement) => {
                            const nodeToRemove = this.nodes.find(
                                (n) => n.definition === flowElement
                            );
                            if (nodeToRemove) {
                                this.removeNode(nodeToRemove);
                            }
        ...
        keydownListener(event) {
            const focusIsOutsideDiagram = !event.target
                .toString()
                .toLowerCase()
                .includes("body");
        ...
        validateDropTarget({ clientX, clientY, control }) {
            const { allowDrop, poolTarget } = getValidationProperties(
                clientX,
                clientY,
                control,
                this.paperManager.paper,
                this.graph,
                this.collaboration,
                this.$refs["paper-container"]
            );
        ...
        setShapeStacking(shape) {
            if (
                this.isRendering ||
                (!shape.component.node.isType("processmaker-modeler-pool") &&
                    shape === this.previouslyStackedShape)
            ) {
                return;
            }
            ...
            this.paperManager.performAtomicAction(() =>
                ensureShapeIsNotCovered(shape, this.graph)
            );
        },
    ...
    mounted() {
        ...
        this.paperManager = PaperManager.factory(
            this.$refs.paper,
            this.graph.get("interactiveFunc"),
            this.graph
        );
        this.paper = this.paperManager.paper;

        this.paperManager.addEventHandler(
        ...
        window.ProcessMaker.EventBus.$emit("modeler-start", {
            ...
            addBreadcrumbs: (breadcrumbs) =>
                this.breadcrumbData.push(breadcrumbs),
        });
    },
};
</script>
