<template>
  <b-container fluid class="h-100 p-0" id="screen-builder-container">
    <b-card no-body class="bg-white border-top-0 h-100" id="app">
      ...
      <!-- Card Body -->
      <b-card-body class="p-0 m-0 overflow-auto">
        <!-- Vue-form-builder -->
        <vue-form-builder
        ...
        <!-- Preview -->
        <b-row class="m-0 h-100" id="preview" v-show="displayPreview" data-cy="preview">
          ...
          <b-col class="p-0 overflow-hidden h-100 preview-inspector">
            <b-card no-body class="p-0 h-100 rounded-0 border-top-0 border-right-0 border-bottom-0">
              <b-card-body class="p-0 overflow-auto">
                <b-button variant="outline"
                  class="text-left shadow-none card-header d-flex align-items-center w-100"
                  @click="showDataInput = !showDataInput"
                >
                  <i class="mr-2 fas fa-file-import"/>
                  {{ $t('Data Input') }}
                  <i class="ml-auto fas" :class="showDataInput ? 'fa-angle-down' : 'fa-angle-right'"/>
                </b-button>
                ...
                >
                  <i class="mr-2 fas fa-file-code"/>
                  {{ $t('Data Preview') }}
                  <i class="ml-auto fas" :class="showDataPreview ? 'fa-angle-down' : 'fa-angle-right'"/>
                </b-button>
      ...
      <!-- Card Footer -->
      <b-card-footer class="d-flex justify-content-end align-items-center">
        ...
        <div v-if="showValidationErrors" class="overflow-auto validation-panel position-absolute border-left border-top" :class="{'d-block':showValidationErrors && validationErrors.length}" data-cy="validation-panel">
          <div v-if="!previewInputValid" class="p-3 font-weight-bold text-dark">
            <i class="mr-3 fas fa-times-circle text-danger"/>
            {{ $t('Invalid JSON Data Object') }}
          </div>
          <b-button variant="link" class="p-3 validation__message d-flex align-items-center"
            v-for="(validation,index) in warnings"
            :key="index"
            @click="focusInspector(validation)"
            data-cy="focus-inspector"
          >
            <i class="mr-3 fas fa-exclamation-triangle text-warning d-block"/>
            <span class="ml-2 text-left text-dark font-weight-bold">
          ...
          <b-button variant="link" class="p-3 validation__message d-flex align-items-center"
            ...
            data-cy="focus-inspector"
          >
            <i class="mr-3 fas fa-times-circle text-danger d-block"/>
            <span class="ml-2 text-left text-dark font-weight-bold">
