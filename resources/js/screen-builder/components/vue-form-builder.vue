    ...
    >
      <b-input-group size="sm" class="bg-white mt-3">
        ...
        <b-button-group size="sm" class="ml-1 ml-auto">
          <b-button @click="undo" :disabled="!canUndo" data-cy="toolbar-undo">{{
            $t("Undo")
          }}</b-button>
          <b-button @click="redo" :disabled="!canRedo" data-cy="toolbar-redo">{{
            $t("Redo")
          }}</b-button>
        </b-button-group>
        ...
        <b-card-body class="p-0 h-100 overflow-auto">
          <template v-for="accordion in accordions">
            <b-button
              ...
              :data-cy="
                `accordion-${accordionName(accordion).replace(' ', '')}`
              "
              :accordion-name="
                `accordion-${accordionName(accordion).replace(' ', '')}`
              "
...
export default {
  ...
  methods: {
    loadVariablesTree() {
      ...
      this.variablesTree = this.$refs.treeOfVariables.getVariablesTree(
        definition
      );
